...
// Main event handler
    ...
    async function admitWA(e: MouseEvent): Promise<void>
    {
        ...
        const storedApplications: Switcher[] = Array.isArray(storedValue) ? storedValue : [];
        const now = Date.now();
        ...
        if (JSON.stringify(storedSwitchers) !== JSON.stringify(storedApplications))
    ...
    function onStorageChange(changes: object): void
    {
        for (let key in changes) {
            ...
            if (key === 'switchers') {
                const newSwitchers: Switcher[] = Array.isArray(storageChange.newValue) ? storageChange.newValue : [];
                const now = Date.now();
                (document.querySelector('#num-switchers') as HTMLSpanElement).innerHTML = String(
                    newSwitchers.filter((switcher) => Number(switcher.expiresAt) > now).length);
            }
    ...
    const activeApplications = storedApplications
        ...
        .filter((switcher) => Number(switcher.expiresAt) > now);
    if (JSON.stringify(activeApplications) !== JSON.stringify(storedApplications))
